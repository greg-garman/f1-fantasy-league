...
router.post('/:raceId', (req: Request, res: Response): void => {
  ...
  const savePicks = db.transaction(() => {
    if (pole) {
      upsertPick.run(userId, raceId, 'pole', pole);
    }
    if (winner) {
      upsertPick.run(userId, raceId, 'winner', winner);
    }
    if (fastestLap) {
      upsertPick.run(userId, raceId, 'fastest_lap', fastestLap);
    }
    if (podium && Array.isArray(podium)) {
      if (podium.length > 3) {
        throw new Error('Maximum 3 podium picks allowed');
      }
      upsertPick.run(userId, raceId, 'podium', JSON.stringify(podium));
    }
    if (dnf) {
      upsertPick.run(userId, raceId, 'dnf', dnf);
    }
    if (h2h && typeof h2h === 'object') {
      upsertPick.run(userId, raceId, 'h2h', JSON.stringify(h2h));
    }
    if (constructorPodium) {
      upsertPick.run(userId, raceId, 'constructor_podium', constructorPodium);
    }
  });
  ...
  // Return saved picks
  const picks = db.prepare('SELECT * FROM weekly_picks WHERE user_id = ? AND race_id = ?').all(userId, raceId) as WeeklyPick[];
...
router.get('/:raceId/all', (req: Request, res: Response): void => {
  const raceId = parseInt(req.params.raceId, 10);
...
router.get('/:raceId/matchups', (req: Request, res: Response): void => {
  const raceId = parseInt(req.params.raceId, 10);
